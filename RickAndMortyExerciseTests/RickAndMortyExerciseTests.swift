//
//  RickAndMortyExerciseTests.swift
//  RickAndMortyExerciseTests
//
//  Created by Kevin Tarr on 2/20/25.
//

import XCTest
@testable import RickAndMortyExercise

@MainActor
final class CharacterViewModelTests: XCTestCase {
    func testSearchCharacters() async throws {
        let viewModel = CharacterViewModel()
        viewModel.searchText = "rick"
        await viewModel.searchCharacters()
        XCTAssertFalse(viewModel.characters.isEmpty, "Expected characters to be fetched")
    }
    
    func testFilterByStatus() async throws {
        let viewModel = CharacterViewModel()
        await MainActor.run {
            viewModel.selectedStatus = "Alive"
            viewModel.searchText = "rick"
        }
        await viewModel.searchCharacters()
        XCTAssertTrue(viewModel.characters.allSatisfy { $0.status == "Alive" })
    }
    
    func testClearFilters() async throws {
        let viewModel = CharacterViewModel()
        await MainActor.run {
            viewModel.selectedStatus = "Alive"
            viewModel.selectedSpecies = "Human"
            viewModel.selectedType = "Disease"
        }
        viewModel.clearFilters()
        XCTAssertEqual(viewModel.selectedStatus, "")
        XCTAssertEqual(viewModel.selectedSpecies, "")
        XCTAssertEqual(viewModel.selectedType, "")
    }
}


final class RickAndMortyExerciseTests: XCTestCase {

    override func setUpWithError() throws {
        // Put setup code here. This method is called before the invocation of each test method in the class.
    }

    override func tearDownWithError() throws {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
    }

    func testExample() throws {
        // This is an example of a functional test case.
        // Use XCTAssert and related functions to verify your tests produce the correct results.
        // Any test you write for XCTest can be annotated as throws and async.
        // Mark your test throws to produce an unexpected failure when your test encounters an uncaught error.
        // Mark your test async to allow awaiting for asynchronous code to complete. Check the results with assertions afterwards.
    }

    func testPerformanceExample() throws {
        // This is an example of a performance test case.
        self.measure {
            // Put the code you want to measure the time of here.
        }
    }

}
